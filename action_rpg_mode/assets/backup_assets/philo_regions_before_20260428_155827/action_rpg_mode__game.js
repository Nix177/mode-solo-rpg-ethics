...
const ASSET_VERSION = "20260428_directionpatch1";
...
function createEnemies(world, count) {
  ...
  const safePoints = [world.spawn, ...world.npcs.map((npc) => ({ x: npc.x, y: npc.y })), ...world.merchants.map((m) => ({ x: m.x, y: m.y }))];
...
function spawnEliteForNpc(npc) {
  ...
  addNotification(`Alerte: menace activÃƒÂ©e prÃƒÂ¨s de ${npc.name}.`, 3.4, "#ffc857");
}
...
function updateHud() {
  ...
  DOM.hudLevel.innerHTML = `Lvl: <strong>${p.level}</strong>${(p.talentPoints || 0) > 0 ? `<span class="talent-plus" title="Points ÃƒÂ  dÃƒÂ©penser dans le grimoire">+${p.talentPoints}</span>` : ""}`;
...
function updateGuideText() {
  if (!state.currentScene) {
    DOM.guideText.textContent = "Choisis ton hÃƒÂ©ros et ta classe pour commencer.";
    return;
  }
...
function onEnemyDefeated(enemy) {
  ...
  if (enemy.challengeNpcId) {
    ...
    addNotification("Zone stabilisÃƒÂ©e. Sceau obtenu.", 2.8, "#67f0c8");
    updateGuideText();
  }
  updateHud();
}
...
function buildNpcLine(npc) {
  ...
  const context = scene.narrative.context || "Ce conflit demande une dÃƒÂ©cision nuancÃƒÂ©e.";
  const statements = [
...
function openNpcChat(npc) {
  ...
  if (!state.quest.talked.has(npc.id)) {
    ...
    addNotification(`Conseiller consultÃƒÂ©: ${npc.name}.`, 2.2, "#67f0c8");
    updateHud();
    updateGuideText();
  }
}
...
function buyPotion() {
  ...
  p.potions += 1;
  playSfx("itemUse", 0.5);
  addNotification("Potion soin achetÃƒÂ©e.", 1.9, "#67f0c8");
  updateHud();
  renderInventory();
}
...
function buyManaPotion() {
  ...
  p.manaPotions += 1;
  playSfx("itemUse", 0.5);
  addNotification("Potion mana achetÃƒÂ©e.", 1.9, "#67f0c8");
  updateHud();
  renderInventory();
}
