...
const ASSET_VERSION = "20260514_shop_sell1";
...
const ZONE_CHIEF_IDLE_SHEETS = {
  village: "./assets/region_assets/npcs/main/processed_idle/village_chief_male_transparent.png",
  mine: "./assets/region_assets/npcs/main/processed_idle/mine_chief_male_transparent.png",
  forest: "./assets/region_assets/npcs/main/processed_idle/forrest_chief_female_transparent.png",
...
const TUTORIAL_STEPS = [
  ...
  { title: "Skill principal", body: "La touche F sert au premier skill de classe, mais il se dÃ©bloque au niveau 3. Au dÃ©but, utilise surtout ESPACE et ta barre rapide.", keys: ["KeyF"], label: "Appuie sur F" },
...
function getPlayerSheetPath(classId, action) {
  const combatActions = new Set(["attack_basic", "skill_1", "skill_2"]);
  if (combatActions.has(action)) {
    return `./assets/imagegen_generated/directional_combat_v1/${classId}/${action}/${classId}_${action}_8dir_sheet_v1.png`;
  }
...
function createWorldShell(scene, region, biomeOverride = null) {
  ...
  return {
    ...
    enemyZones: [],
    safeZones: [],
    enemyRespawn: { interval: 120, nextAt: null, batch: 4 },
    enemyTargetCount: 0,
...
function addFactionNpcForRegion(world, scene, region) {
  ...
  const npc = {
    ...
    idleSheetPath: getChiefNpcIdleSheetPath(region),
    isZoneChief: true,
    questGiver: true,
...
function addVillageMaster(world, region) {
  ...
  world.master = {
    ...
    dialogue: {
      ...
      counter: "Certains chemins sont risquÃ©s, mais comprendre le village demande de sortir de la place centrale.",
...
function addVillageManualProps(world) {
  ...
  const plaza = { x: w * 0.50, y: h * 0.48 };

  addPlacedProp(world, "village_market_stall", plaza.x - 230, plaza.y - 110, 82, 72, { footprint: { w: 58, h: 20, y: 20 }, occludesPlayer: false });
  addPlacedProp(world, "village_market_stall", plaza.x + 230, plaza.y - 104, 82, 72, { footprint: { w: 58, h: 20, y: 20 }, occludesPlayer: false });
  addPlacedProp(world, "village_bench", plaza.x - 155, plaza.y + 190, 74, 44, { footprint: { w: 58, h: 18, y: 10 }, occludesPlayer: false });
  addPlacedProp(world, "village_bench", plaza.x + 155, plaza.y + 190, 74, 44, { footprint: { w: 58, h: 18, y: 10 }, occludesPlayer: false });
  addPlacedProp(world, "village_lamp_post", plaza.x - 330, plaza.y - 210, 42, 86, { footprint: { w: 24, h: 20, y: 28 }, occludesPlayer: true });
  addPlacedProp(world, "village_lamp_post", plaza.x + 330, plaza.y - 210, 42, 86, { footprint: { w: 24, h: 20, y: 28 }, occludesPlayer: true });
  addPlacedProp(world, "village_lamp_post", plaza.x - 330, plaza.y + 235, 42, 86, { footprint: { w: 24, h: 20, y: 28 }, occludesPlayer: true });
  addPlacedProp(world, "village_lamp_post", plaza.x + 330, plaza.y + 235, 42, 86, { footprint: { w: 24, h: 20, y: 28 }, occludesPlayer: true });

  addPlacedProp(world, "village_cart", w * 0.37, h * 0.53, 96, 68, { footprint: { w: 76, h: 28, y: 16 }, occludesPlayer: false });
  addPlacedProp(world, "village_water_trough", w * 0.75, h * 0.60, 86, 48, { footprint: { w: 70, h: 22, y: 10 }, occludesPlayer: false });
  addPlacedProp(world, "village_signpost", w * 0.57, h * 0.66, 52, 86, { footprint: { w: 28, h: 20, y: 28 }, occludesPlayer: false });

  addPlacedProp(world, "village_barrel_stack", w * 0.22, h * 0.54, 68, 66, { footprint: { w: 48, h: 24, y: 18 }, occludesPlayer: false });
  addPlacedProp(world, "village_crate_stack", w * 0.25, h * 0.54, 70, 64, { footprint: { w: 50, h: 24, y: 18 }, occludesPlayer: false });
  addPlacedProp(world, "village_chopping_block", w * 0.31, h * 0.67, 66, 56, { footprint: { w: 46, h: 20, y: 14 }, occludesPlayer: false });
  addPlacedProp(world, "village_sacks", w * 0.63, h * 0.55, 72, 52, { footprint: { w: 52, h: 20, y: 12 }, occludesPlayer: false });
  addPlacedProp(world, "village_barrel_stack", w * 0.70, h * 0.55, 68, 66, { footprint: { w: 48, h: 24, y: 18 }, occludesPlayer: false });

  addPlacedProp(world, "village_flower_planter", w * 0.43, h * 0.35, 58, 42, { collidable: false, occludesPlayer: false });
  addPlacedProp(world, "village_flower_planter", w * 0.57, h * 0.35, 58, 42, { collidable: false, occludesPlayer: false });
  addPlacedProp(world, "village_fence_gate", w * 0.82, h * 0.46, 84, 46, { footprint: { w: 70, h: 18, y: 10 }, occludesPlayer: false });
  addPlacedProp(world, "village_fence_gate", w * 0.14, h * 0.45, 84, 46, { footprint: { w: 70, h: 18, y: 10 }, occludesPlayer: false });
}
...
function decorateRegionWorld(world) {
  ...
  if (region.type === "village") {
    ...
    world.safeZones = [
      { id: "village_centre", label: "Centre du village", enemyFree: true, x: hub.x, y: hub.y, r: Math.min(world.width, world.height) * 0.31 },
      { id: "village_north_houses", label: "Maisons du village", enemyFree: true, x: world.width * 0.18, y: world.height * 0.08, w: world.width * 0.66, h: world.height * 0.23 }
    ];
...
function getRegionContactName(regionId) {
  const region = getRegionDefinition(regionId);
  const chars = state.currentScene?.narrative?.characters || [];
  const char = typeof region?.characterIndex === "number" ? chars[region.characterIndex] : null;
  return fixText(char?.name || region?.shortLabel || region?.label || "chef de zone");
}

function getAcceptedQuestList() {
  return Object.values(state.quest.acceptedQuests || {}).sort((a, b) => a.time - b.time);
}

function getQuestRecordForNpc(npc, kind) {
  ...
  if (!region || region.id === config.start) return null;
  if (kind === "main") {
    return {
      id: `region_${region.id}_main`,
      type: "region-main",
      npcId: npc.id,
      regionId: region.id,
      title: region.questTitle || `Comprendre ${region.label}`,
      desc: region.questMain || `Parler a ${npc.name} pour comprendre ce point de vue.`,
      time: Date.now()
    };
  }
  if (kind === "optional") {
    return {
      id: `region_${region.id}_optional`,
      type: "region-optional",
      npcId: npc.id,
      regionId: region.id,
...
function questIsDone(quest) {
  if (!quest) return false;
  const config = getSceneRegionConfig();
  if (quest.type === "explore") {
    const required = config ? getRequiredFactionRegions() : [];
    return required.length > 0 && required.every((regionId) => getRegionProgress(regionId).talked);
  }
  const progress = getRegionProgress(quest.regionId);
  if (quest.type === "region-main") return progress.talked;
  if (quest.type === "region-optional") return progress.optional;
  return false;
}
...
function getQuestObjectives(quest) {
  ...
  if (quest.type === "explore") {
    return getRequiredFactionRegions().map((regionId) => {
      const region = getRegionDefinition(regionId);
      const progress = getRegionProgress(regionId);
  ...
  const region = getRegionDefinition(quest.regionId);
  const progress = getRegionProgress(quest.regionId);
  if (quest.type === "region-main") {
    return [
      { text: `Rejoindre ${region?.label || quest.regionId}.`, done: progress.visited },
      { text: `Parler a ${getRegionContactName(quest.regionId)}.`, done: progress.talked }
    ];
  }
  if (quest.type === "region-optional") {
    const challenge = state.quest.challenges?.[quest.npcId];
    return [
      { text: `Accepter l'aide locale de ${getRegionContactName(quest.regionId)}.`, done: true },
      { text: challenge?.active ? "Stabiliser la menace apparue dans la zone." : "Stabiliser la zone dangereuse.", done: progress.optional }
    ];
  }
  return [{ text: quest.desc || quest.title, done: questIsDone(quest) }];
}

function acceptQuestFromNpc(kind) {
  const npc = state.chatNpc;
  if (!npc) return;
...
function renderQuestTracker() {
  ...
  if (!quests.length) {
    DOM.questTracker.classList.add("hidden");
...
function createEnemies(world, count, options = {}) {
  const level = world.levelNumber;
  if (options.setTarget !== false) world.enemyTargetCount = Math.max(world.enemyTargetCount || 0, count);
...
function bossNameForWorld(world) {
  const names = { forest: "Gardien du chantier", lab: "Prototype instable", hospital: "Crise logistique", space: "Sentinelle de dÃ´me", coast: "Gardien des marÃ©es", urban: "Chef de patrouille" };
...
function skillMeta(skillKey) {
  ...
  const fallback = skillKey === "skill1" ? "CompÃ©tence F" : "CompÃ©tence R";
  return { key: skillKey, label: def?.name || fallback, desc: def?.desc || "CompÃ©tence de classe.", icon: def?.icon || `./assets/talents/${skillKey === "skill2" ? "special" : "skill1"}.svg`, unlocked: skillKey === "skill1" ? !!p?.unlockedSkills?.skill1 : !!p?.unlockedSkills?.skill2, levelReq: skillKey === "skill1" ? 3 : (def?.levelReq || 3) };
}
...
function assignHotbarSlot(slotIndex, payload) {
  ...
  renderHotbar();
  addNotification(`Raccourci ${slotIndex + 1} mis Ã  jour.`, 1.6, "#67f0c8");
}
...
function renderSkillBook() {
  ...
  for (const entry of entries) {
    ...
    const lockText = entry.unlocked ? (entry.count !== undefined ? `Stock: ${entry.count}` : "DÃ©bloquÃ©") : `DÃ©bloquÃ© au niveau ${entry.levelReq}`;
...
function resetQuestProgress() {
  ...
  state.quest.acceptedQuests = {};
  state.quest.trackedQuestId = null;
...
function updateHud() {
  ...
  DOM.hudHero.textContent = `HÃ©ros: ${p.heroName}`;
...
function pointInEnemySafeZone(world, x, y, margin = 0) {
  if (!world?.safeZones?.length) return false;
  return world.safeZones.some((zone) => {
    if (!zone.enemyFree) return false;
    if (zone.r) return Math.hypot(x - zone.x, y - zone.y) <= zone.r + margin;
    if (typeof zone.x === "number" && typeof zone.y === "number" && typeof zone.w === "number" && typeof zone.h === "number") {
      return x >= zone.x - margin && x <= zone.x + zone.w + margin && y >= zone.y - margin && y <= zone.y + zone.h + margin;
    }
    return false;
  });
}

function pushEnemyOutOfSafeZones(enemy, world = state.world) {
  if (!world?.safeZones?.length || enemy.boss) return false;
  let pushed = false;
  for (const zone of world.safeZones) {
    if (!zone.enemyFree || !zone.r) continue;
    const dx = enemy.x - zone.x;
    const dy = enemy.y - zone.y;
    const minDist = zone.r + (enemy.r || 12) + 8;
    const dist = Math.hypot(dx, dy);
    if (dist >= minDist) continue;
    const nx = dist > 0.001 ? dx / dist : 1;
    const ny = dist > 0.001 ? dy / dist : 0;
    enemy.x = clamp(zone.x + nx * minDist, (enemy.r || 12), world.width - (enemy.r || 12));
    enemy.y = clamp(zone.y + ny * minDist, (enemy.r || 12), world.height - (enemy.r || 12));
    enemy.vx = nx;
    enemy.vy = ny;
    enemy.wanderT = 0.25;
    pushed = true;
...
function scheduleEnemyRespawn(world = state.world, now = performance.now() / 1000) {
  if (!world?.enemyRespawn) return;
  const next = now + (world.enemyRespawn.interval || 120);
  world.enemyRespawn.nextAt = world.enemyRespawn.nextAt ? Math.min(world.enemyRespawn.nextAt, next) : next;
}

function countAliveRespawnableEnemies(world = state.world) {
  if (!world?.enemies) return 0;
  return world.enemies.filter((enemy) => enemy.hp > 0 && !enemy.boss && !enemy.elite && !enemy.challengeNpcId).length;
}

function updateEnemyRespawns(now) {
  const world = state.world;
  if (!world?.enemyRespawn || !world.enemyTargetCount) return;
  if (!world.enemyRespawn.nextAt || now < world.enemyRespawn.nextAt) return;
  const alive = countAliveRespawnableEnemies(world);
  const missing = Math.max(0, world.enemyTargetCount - alive);
  if (missing > 0) {
    const batch = Math.min(missing, world.enemyRespawn.batch || missing);
    createEnemies(world, batch, { setTarget: false, respawn: true });
  }
  const stillMissing = Math.max(0, world.enemyTargetCount - countAliveRespawnableEnemies(world));
  world.enemyRespawn.nextAt = stillMissing > 0 ? now + 12 : null;
}
...
function useSkill1(now) {
  ...
  if (!p.unlockedSkills?.skill1) {
    addNotification("Skill F verrouillÃ©: atteins le niveau 3 pour le dÃ©bloquer dans le grimoire.", 2.4, "#c084fc");
    return;
  }
  const stats = getEffectiveStats();
...
function useSkill2(now) {
  ...
  if (!p.unlockedSkills || !p.unlockedSkills.skill2) {
    addNotification("Skill R verrouillÃ©: dÃ©bloque le talent spÃ©cial dans le grimoire.", 2.2, "#c084fc");
    return;
  }
  const stats = getEffectiveStats();
...
function maybeLevelUp() {
  ...
  if (leveled) {
    ...
    addNotification(`Niveau ${p.level} atteint! ${p.talentPoints} point(s) de talent Ã  dÃ©penser dans le grimoire.`, 3.8, "#ffc857");
    if (unlockedSkill1) addNotification("CompÃ©tence F dÃ©bloquÃ©e! Glisse-la depuis le grimoire vers la barre rapide.", 3.4, "#c084fc");
  }
  renderInventory();
  renderGrimoire();
}
...
function damagePlayer(rawDamage, now) {
  ...
  if (p.hp <= 0) {
    ...
    addNotification("DÃ©faite temporaire. Repli au camp.", 3.2, "#ff8c8c");
  }
}
...
function updateEnemies(dt, now) {
  ...
  for (const enemy of state.world.enemies) {
    ...
    const dist = Math.hypot(dx, dy);
    const playerInSafeZone = pointInEnemySafeZone(state.world, p.x, p.y, p.r || 14);
    if (pointInEnemySafeZone(state.world, enemy.x, enemy.y, enemy.r || 12)) pushEnemyOutOfSafeZones(enemy, state.world);
    let vx = 0;
    let vy = 0;

    const homeDist = Math.hypot(enemy.x - (enemy.homeX || enemy.x), enemy.y - (enemy.homeY || enemy.y));
    if (homeDist > (enemy.leash || 620) || playerInSafeZone) {
    ...
    pushEnemyOutOfSafeZones(enemy, state.world);
    ...
    if (!playerInSafeZone && dist <= enemy.range + p.r && now - enemy.lastAttack > 0.95) {
...
function drawGate(camX, camY) {
  ...
  if (Math.hypot(state.player.x - gate.x, state.player.y - gate.y) < 90) {
    ...
    ctx.fillText(gate.open ? "[E] Voter et quitter" : "Porte verrouillÃ©e", x - 24, y - 17);
  }
}
...
function drawNpcSpriteEntity(entity, camX, camY, fallbackColor = "#ffd166") {
  const x = entity.x - camX;
  const y = entity.y - camY;
  if (entity.idleSheetPath && state.assets.images[entity.idleSheetPath]) {
    return drawFromSpritesheet(entity.idleSheetPath, "idle", "down", x, y, Math.max(64, (entity.r || 20) * (entity.spriteScale || 4.7)), 4.6, {
      cols: 6,
      rows: 1,
      frameCount: 6,
      rowMap: { down: 0 },
...
function openMasterCouncil() {
  ...
  if (!allRequiredFactionsHeard()) {
    openNpcChat(state.world.master);
    return;
  }
  openVotePanel();
}
...
function usePotion() {
  ...
  if (p.hp >= stats.maxHp) { addNotification("Vie dÃ©jÃ  au maximum.", 1.5, "#ffc857"); return; }
  p.potions -= 1;
  p.hp = Math.min(stats.maxHp, p.hp + Math.round(stats.maxHp * 0.45));
  playSfx("itemUse", 0.55);
  addNotification("Potion soin utilisÃ©e.", 1.8, "#67f0c8");
  updateHud();
  renderInventory();
}
...
function useManaPotion() {
  ...
  if (p.mana >= stats.maxMana) { addNotification("Mana dÃ©jÃ  au maximum.", 1.5, "#ffc857"); return; }
  p.manaPotions -= 1;
  p.mana = Math.min(stats.maxMana, p.mana + Math.round(stats.maxMana * 0.55));
  playSfx("itemUse", 0.55);
  addNotification("Potion mana utilisÃ©e.", 1.8, "#67f0c8");
  updateHud();
  renderInventory();
}
...
function advanceToNextScene(preferredTargetId) {
  ...
  if (!nextId) {
    addNotification("Campagne terminÃ©e. Bravo mÃ©diateur.", 4, "#67f0c8");
    return;
  }
...
function submitVote() {
  ...
  if (justification.length < 20) {
    addNotification("Justification trop courte (20+ caractÃ¨res).", 2.4, "#ff8c8c");
    return;
  }
  ...
  closeVotePanel();
  addNotification("Vote enregistrÃ©. Transition en cours...", 2.8, "#67f0c8");
...
function applyCharacterChange() {
  ...
  DOM.classPanel.classList.add("hidden");
  DOM.startBtn.textContent = "DÃ©marrer";
  state.paused = false;
  state.overlay = null;
  updateHud();
  renderInventory();
...
function setupEvents() {
  ...
  DOM.chatChoices?.addEventListener("click", (event) => {
    const questButton = event.target.closest("button[data-quest]");
    if (questButton) {
      acceptQuestFromNpc(questButton.dataset.quest);
      return;
    }
